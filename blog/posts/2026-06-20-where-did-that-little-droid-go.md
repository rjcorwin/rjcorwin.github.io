...
For a while I assumed model watching model was a harm reduction play for the crowd running with permissions turned off. I do not think that anymore. It looks more like the actual direction. The friction goes down, the autonomy goes up, the trust goes up, and a model keeps watch so that all of that is survivable. There is a research thread behind this too, the AI control work on a trusted monitor watching a less trusted worker. The neat result there is that the watcher does not need to be smarter than the worker. It needs to be trusted and it needs to be independent.
...
This has a name now, apparently. Loop engineering. You drop an agent in a while loop and let it run, then you put another loop on top watching that one, and another layer above that, all of them mostly there to make sure nobody gets stuck. In my setup it is Claude Code instances all the way down. One of them is pretending to be me, the orchestrator. Under it are the task specific agents doing the real work. They control each other and they talk to each other over tmux sessions. So one agent watching another is not new to me. I already do it, just crudely, to keep things from seizing up. What I am reaching for here is a watcher that is paying attention to something better than stuck.
