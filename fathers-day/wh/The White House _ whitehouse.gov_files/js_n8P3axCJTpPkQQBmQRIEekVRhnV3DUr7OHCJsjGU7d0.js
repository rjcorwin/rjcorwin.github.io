...
(function ($) {
  /**
   * Creates a pym child object connected to the original iframe id.
   */
  Drupal.behaviors.wh_pym = {
    attach: function (context, settings) {
      $('.node-type-webform ', context).once('wh-pym', function () {
        var pymChild = new pym.Child({
          xdomain: '.*whitehouse\.gov',
          id: settings.wh_pym.parent_id
        });
        // Sometimes the form height is not calculated correctly right away.
        // Resend the height after the clientside validation is initialized,
        // the delay appears to be enough to allow everything on the form to
        // load.
        $(document).on('clientsideValidationInitialized', function() {
          $('form').trigger('formHeightChanged');
        });
        $(document).on('clientsideValidationFormHasErrors', function() {
          $('form').trigger('formHeightChanged');
        });
        $(document).on('focusout', function() {
          $('form').trigger('formHeightChanged');
        });
         $('form').on('formHeightChanged', function (event) {
          var timeout = window.setTimeout(function () {
            pymChild.sendHeight();
          }, 500);
        });
      });
      $('.node-type-page #page', context).once('wh-pym-scroll', function () {
        var pymChild = new pym.Child({
          xdomain: '.*whitehouse\.gov',
          id: settings.wh_pym.parent_id
        });
        var timeout = window.setTimeout(function () {
          pymChild.scrollParentTo('page');
        }, 500);
      });
    }
  };
