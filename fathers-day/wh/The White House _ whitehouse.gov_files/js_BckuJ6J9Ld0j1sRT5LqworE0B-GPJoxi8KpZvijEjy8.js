...
(function ($) {
  Drupal.behaviors.multibyteExcludeCharacterValidator = {
    attach: function (context) {
      ...
      $(document).bind('clientsideValidationAddCustomRules', function(event){
        ...
        $.validator.addMethod("multibyteExclude", function(value, element, param) {
          // Return true if valid, false if invalid.
          try {
            var regexp = /(?:[\u{10000}-\u{10FFFF}])+/u
            result = regexp.test(value);
          }
          catch(err) {
            // Android, javascript, and unicode don't play well together. Return
            // true and let the server perform the validation.
            return true;
          }
          return !result;
          // Enter a default error message.
        }, $.format('Value cannot contain emoji or symbols.'));
      });
    }
  }
})(jQuery);
